...
window.updateCode = function () {
    // ... 原来 updateCode 函数的实现 ...
    // 用户密钥
    const digitalgd = getSecretBin('123');
    const mingmei = getSecretBin('123');
    ...
    */
    ...
    function getCodeBySecret(secret) {
        ...
        const HS = binaryToArray(CryptoJS.HmacSHA1(CryptoJS.enc.Latin1.parse(T1), CryptoJS.enc.Latin1.parse(secret)).toString(CryptoJS.enc.Latin1))
    ...
    let digitalCode = getCodeBySecret(digitalgd);
    let mingmeiCode = getCodeBySecret(mingmei);
    document.getElementById("digitalgd-code").textContent = digitalCode;
    document.getElementById("mingmei-code").textContent = mingmeiCode;
...
window.updateCode(); // 最初调用 updateCode 函数
